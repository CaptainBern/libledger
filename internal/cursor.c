...
bool ledger_cursor_skip(struct ledger_cursor *cursor, size_t len)
{
...
bool ledger_cursor_copy(struct ledger_cursor *to, struct ledger_cursor *from, size_t len)
{
	...
	if (ledger_cursor_remaining(to) < len)
		return false;

	void *ptr_to = to->buffer.data + to->cursor;
	if (!ptr_to)
		return false;

	void *ptr_from = from->buffer.data + from->cursor;
	if (!ptr_from)
		return false;
...
bool ledger_cursor_read_bytes(struct ledger_cursor *cursor, uint8_t *out, size_t len)
{
	...
	void *ptr = cursor->buffer.data + cursor->cursor;
	if (!ptr)
		return false;
